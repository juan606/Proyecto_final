...
/* ----------------   Práctica 10 --------------------------*/
/*-----------------    2019-2   ---------------------------*/
/*------------- Alumno:   Juan Carlos Nolasco Fuentes                  ---------------*/
...
// settings
// Window size
int SCR_WIDTH = 900000;
int SCR_HEIGHT = 900000;
...
void animate(void)
{
	//if (movKit_z<10.8f|| movKit_r==10.829999) {
		if (20< movKit_r) {
			//if (movKit_r < 0.0f) {
			movKit_z -= R_f_valor;
			if (movKit_z < 0.0f) {
...
void display(Shader shader, Model modelo, Model llantas)
{
	...
	model = glm::translate(model, glm::vec3(15.0f, -1.75f, movKit_z));
...
int main()
{
	...
	//Model ourModel = ((char *)"Models/Cat/cat.obj");
